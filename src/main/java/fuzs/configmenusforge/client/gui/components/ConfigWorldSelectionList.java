...
public class ConfigWorldSelectionList extends CustomBackgroundObjectSelectionList<ConfigWorldSelectionList.ConfigWorldListEntry> {
   ...
   public ConfigWorldSelectionList(SelectConfigWorldScreen selectConfigWorldScreen, Minecraft minecraft, int width, int height, int y0, int y1, int itemHeight, String query, List<LevelSummary> levelList) {
      ...
      this.createConfigTooltip = new TranslatableComponent("configmenusforge.gui.select.create_config", selectConfigWorldScreen.getDisplayName()).withStyle(ChatFormatting.GOLD);
   ...
   protected void moveSelection(SelectionDirection pOrdering) {
      this.moveSelection(pOrdering, (configWorldListEntry) -> !configWorldListEntry.summary.isDisabled());
   }
