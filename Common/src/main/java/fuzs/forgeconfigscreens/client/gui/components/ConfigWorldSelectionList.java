...
public class ConfigWorldSelectionList extends ObjectSelectionList<ConfigWorldSelectionList.ConfigWorldListEntry> {
    ...
    public final class ConfigWorldListEntry extends Entry<ConfigWorldListEntry> implements AutoCloseable {
        ...
        private static void loadModConfig(ModConfig config, Path configBasePath) {
            ...
            CommonAbstractions.INSTANCE.setConfigData(config, configData);
        ...
        @Override
        public void render(GuiGraphics guiGraphics, int pIndex, int top, int left, int pWidth, int pHeight, int pMouseX, int pMouseY, boolean hovered, float pPartialTicks) {
            ...
            guiGraphics.drawString(this.minecraft.font, s, left + 32 + 3, top + 1, 16777215);
            guiGraphics.drawString(this.minecraft.font, s1, left + 32 + 3, top + 9 + 3, 8421504);
            guiGraphics.drawString(this.minecraft.font, component, left + 32 + 3, top + 9 + 9 + 3, 8421504);
            RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
            RenderSystem.enableBlend();
            guiGraphics.blit(this.icon != null ? this.iconLocation : ConfigWorldSelectionList.ICON_MISSING, left, top, 0.0F, 0.0F, 32, 32, 32, 32);
            ...
            if (this.minecraft.options.touchscreen().get() || hovered) {
                ...
                if (this.summary.isLocked()) {
                    guiGraphics.blit(ConfigWorldSelectionList.ICON_OVERLAY_LOCATION, left, top, 96.0F, textureY, 32, 32, 256, 256);
                    if (leftHovered) {
                        this.screen.setActiveTooltip(this.minecraft.font.split(ConfigWorldSelectionList.WORLD_LOCKED_TOOLTIP, 175));
                    }
                } else if (this.summary.requiresManualConversion()) {
                    guiGraphics.blit(ConfigWorldSelectionList.ICON_OVERLAY_LOCATION, left, top, 96.0F, 32.0F, 32, 32, 256, 256);
                    if (leftHovered) {
                        this.screen.setActiveTooltip(this.minecraft.font.split(ConfigWorldSelectionList.WORLD_REQUIRES_CONVERSION, 175));
                    }
                } else if (this.summary.markVersionInList()) {
                    guiGraphics.blit(ConfigWorldSelectionList.ICON_OVERLAY_LOCATION, left, top, 32.0F, textureY, 32, 32, 256, 256);
                    if (this.summary.askToOpenWorld()) {
                        ...
                        guiGraphics.blit(ConfigWorldSelectionList.ICON_OVERLAY_LOCATION, left, top, 64.0F, textureY, 32, 32, 256, 256);
                        if (leftHovered) {
                    ...
                    guiGraphics.blit(ConfigWorldSelectionList.ICON_OVERLAY_LOCATION, left, top, 0.0F, textureY, 32, 32, 256, 256);
                }
            }
        }
